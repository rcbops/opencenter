...
# on the non-admin api, we'll only run the adventure
...
def execute_adventure(adventure_id):
    ...
    try:
        criteria = adventure['criteria']
        if api.apply_expression(data['node'], criteria) is not True:
            return generic.http_badrequest(
                msg='Not a valid adventure for this node: unmet criteria')
